...
class MailFaker {
    ...
    constructor(locale?: string) {
        ...
        // this.locale is guaranteed to be a valid key at this point.
        this.data = locales[this.locale]!;
    }

    _randomItem<T>(arr: T[]): T {
        // Index is always in range for a non-empty array.
    ...
    domainPart(): string {
        const domain = this._randomItem(this.data.domains);
        const tld = this._randomItem(globalTlds);
        return `${domain}.${tld}`;
    }
...
const globalTlds = ["com", "net", "org", "info", "biz", "io", "co", "xyz", "online", "site"];

// Export: publish to the global scope so create-address.ts can consume it.
// (esbuild wraps this module in an IIFE, so a top-level class is not global.)
Object.assign(globalThis, { MailFaker });
