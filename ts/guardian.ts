...
interface PublicKeyFile {
    keys: Record<string, PublicKeyEntry>;
}
...
async function checkEd25519Support(): Promise<boolean> {
    ...
    try {
        ...
        console.warn("[Guardian] Ed25519 NOT supported in this browser:", (err as Error).message);
    }
    return ed25519Supported;
}
...
const publicKeys = new Map<string, KeyInfo>(); // keyId -> { cryptoKey, validFrom, warnAfter, validUntil }
let guardianInitialized = false;
const tabSecurityStatus = new Map<number, TmSecurityStatus>(); // tabId -> { status, reason, verified, failed }
...
self.publicKeys = publicKeys as unknown as Map<string, CryptoKey>;
 ...
 */
const cachedCertFingerprints: Record<string, CachedFingerprint> = {};

/**
 * Flag to prevent concurrent fingerprint fetches (per hostname)
 */
const fetchingFingerprints: Record<string, boolean> = {};
 ...
 */
async function fetchServerFingerprint(hostname: string): Promise<CachedFingerprint | null> {
    ...
    try {
        ...
        const data = JSON.parse(body) as FingerprintApiResponse;
 ...
 */
function getCertFingerprint(cert: GuardianCertificate | undefined): string | null {
 ...
 */
async function checkCertificate(details: chrome.webRequest.OnHeadersReceivedDetails): Promise<void> {
    if (!guardianEnabled) {
        return;
    }
    ...
    try {
        ...
        if (isTrustedIssuer && subjectMatchesDomain) {
            ...
            const status: TmSecurityStatus = tabSecurityStatus.get(details.tabId) || {
                status: "PROTECTED",
                verified: 0,
                unsigned: 0,
                failed: [],
        ...
        // Der Server signiert seine Antwort mit Ed25519; ein MITM kann diese
        // Signatur nicht fälschen (fetchServerFingerprint verifiziert sie).
        const serverInfo = await fetchServerFingerprint(requestHostname);
        if (serverInfo) {
            // Stärkster Beweis: Stimmt der vom Browser gesehene Fingerprint mit dem
            // signierten Server-Fingerprint überein, sieht der Server EXAKT dasselbe
            // Zertifikat wie der Browser → keine Interception, auch bei unbekannter
            // (aber legitimer) CA. Ein unbekannter Issuer ALLEIN ist also KEIN MITM.
            ...
            // Fallback: Der Server liefert issuer nur bei einem Cache-Miss mit. Falls
            // vorhanden, akzeptiere gleiche Issuer-Organisation + passende Domain als
            // legitimes anderes CloudFlare-Edge-Cert derselben CA (der Fingerprint darf
            // dort abweichen, deshalb kein harter Fingerprint-Vergleich).
        ...
        // Echter Fingerprint-Mismatch bei unbekanntem Issuer = MITM
        ...
        console.error("[Guardian] Error details:", (err as Error).message, (err as Error).stack);
    }
}
 ...
 */
function handleMitmDetected(tabId: number, message: string): void {
    // Update tab status
    const status: TmSecurityStatus = tabSecurityStatus.get(tabId) || {
        status: "COMPROMISED",
        verified: 0,
        unsigned: 0,
        failed: [],
 ...
 */
async function loadPublicKeys(): Promise<boolean> {
    try {
 ...
 */
async function verifySignature(body: string, signature: string, timestamp: number, keyId: string): Promise<VerificationResult> {
    // Find key
    ...
    const ageSeconds = Math.abs((now.getTime() - timestampDate.getTime()) / 1000);
 ...
 */
async function processResponse(details: chrome.webRequest.OnResponseStartedDetails): Promise<void> {
    if (!guardianEnabled) {
        return;
    }
 ...
 * Update badge icon
 */
function updateBadge(tabId: number, status: { status?: string } | null | undefined): void {
    if (!status) {
        return;
    }
 ...
 */
const pingInProgress = new Set<number>();
async function pingForVerification(tabId: number, hostname: string): Promise<void> {
    ...
    try {
        ...
        console.log("[Guardian] Ping failed:", (err as Error).message);
    } finally {
 ...
 */
function resetTabStatus(tabId: number): void {
 ...
 */
function showSecurityWarning(tabId: number, message: string): void {
    ...
    browser.tabs.sendMessage(tabId, {
        ...
        browser.scripting.executeScript({
            target: { tabId: tabId },
            func: (warningMessage: string, title: string, dismiss: string) => {
 ...
 */
async function initGuardian(): Promise<void> {
    ...
    if (isFirefox()) {
        ...
        browser.webRequest.onHeadersReceived.addListener(
            checkCertificate as unknown as (details: chrome.webRequest.OnHeadersReceivedDetails) => chrome.webRequest.BlockingResponse | undefined,
            {
                urls: [
                    "*://mail.aionda.com/*",
                    "*://*.mail.aionda.com/*",
...
// Guardian is OPT-IN: disabled by default because antivirus software and
// corporate proxies that intercept HTTPS traffic trigger false MITM alarms.
// The user has to enable it explicitly in the extension options.
let guardianEnabled = false;
self.guardianEnabled = false;

function startGuardianIfEnabled(): void {
...
// React to the option being toggled while the service worker is alive
browser.storage.onChanged.addListener((changes, area) => {
    ...
    if (changes["guardian_enabled"]?.newValue) {
        ...
        self.guardianEnabled = false;
        // Clear all Guardian badges
        browser.tabs.query({}).then(tabs => {
            for (const tab of tabs) {
                try { browser.action.setBadgeText({ tabId: tab.id, text: "" }); } catch (e) { /* tab gone */ }
            }
        });
    }
});
