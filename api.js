...
function callAPI(data, json=null) {
    ...
    return fetch(API_BASE_URL + "/?api=1&" + params.toString(), data).then(function (response) {
        if (response.ok)
        ...
        if (response["data"] && response["data"]["requires_2fa"]) {
            let error = new Error(response["data"]["pat_hint"] || msg || "2FA required. Please create a Personal Access Token in the TrashMail Manager and use it as password.");
            error.requires_2fa = true;
            error.url = response["data"]["url"];
            error.extension_html = response["data"]["extension_html"];
            throw error;
        }
